...
- High-frequency per-action student state (BKT mastery, behavior/Elo history) is **not** stored relationally — it's cached in Redis and flushed as JSON into `branch.conceptMapState` and `userprofile.strengthWeaknessMatrix` to avoid write amplification. **Mastery is per branch, not per user**: the same skill required by two goals tracks progress separately in each branch, so read it from the branch the request is scoped to, never from `userprofile`. Curriculum metadata (`skill`, `skillPrerequisite`, `goalSkillRequire`) stays relational because staff need CRUD access via the admin dashboard.
- **Prerequisite unlock rule**: a skill unlocks only when *all* its parent prerequisites (from `skillPrerequisite`) have reached **P(L) ≥ 0.95 — the same as Progress = 100%**, because Progress is never rounded up ([ADR 0004](docs/adr/0004-progress-two-decimals-truncated.md)) — in that branch's `conceptMapState` — the frontend's `computeUnlockedSkills`/`canUnlockSkill` check `=== 100`, and `recommendNextSkill`/`getBranchStats` check `pL >= 0.95`. (Earlier versions of this file said ≥60%; no code implements that.)
- **Goal node** ([ADR 0005](docs/adr/0005-goal-node-derived-on-read.md)): every branch's skill tree ends in a goal node, derived on every read of `GET /branch/:branchId/skills` by `buildGoalNode` (`libs/bkt/goalNode.ts`) and **never stored** — no row, migration or branch-creation hook, so every branch, new or old, has one. The endpoint returns `{ skills, goal }`; keep the goal node out of the skill array, or unlock/picker/exercise/recommendation code will treat it as a practisable skill. It is complete when every `goalSkillRequire` skill has P(L) ≥ 0.95 in that branch (the unlock test) — this repo counts (`masteredCount`/`requiredCount`/`isComplete`); the frontend in `../G06_adaptive_learning` only draws it. Completion is **sticky**: the first time it holds, `branch.goalCompletedAt` is stamped (by `submitAnswer` and pretest seeding) and the goal stays complete even if a skill drops later; only the answer that caused it returns `summary.goalCompleted`, which the session summary celebrates. Goal progress (average Progress of the required skills, not-started = 0) has one implementation, `goalMastery`, shared by the goal node and `getBranchStats.goalProgressPercent` — don't compute it a second way.
- **Practice-session drafts** ([ADR 0003](docs/adr/0003-practice-session-drafts.md)): an unfinished `session` (`endedAt` null) is the student's draft for that skill in that branch, and `/session/start` resumes it instead of opening a new one. There is at most one per (branch, skill) — `pickDraft` in `libs/session/sessionDraft.ts`; the rest are closed as `abandoned` — and the same question comes back because `QuestionSelector.selectNext` is deterministic. Keep it deterministic, or a resumed draft shows a different question. Pretest sessions have `branchId`/`skillId` null and are never drafts.
- **Students see Progress, never raw P(L)** ([ADR 0001](docs/adr/0001-students-see-progress-not-pl.md); terms in [`docs/glossary.md`](docs/glossary.md)). Progress = `pL / 0.95 × 100`, capped at 100, **2 decimals, truncated — never rounded up** ([ADR 0004](docs/adr/0004-progress-two-decimals-truncated.md)), so it reads 100% exactly when P(L) ≥ 0.95. It is computed only by `MasteryState.progressOf` and re-derived from `pL` on every read (`getEntry`), so the stored value is just a cache; a skill with `attemptCount = 0` is shown as "not started". The skill tree reads it from `getBranchSkills`, the Exercise screen from `progress` on `/session/start` and `/session/:id/answer`. Any new screen that shows skill progress must return `SkillProgress` (`MasteryState.toProgress`) rather than let the frontend recompute from `pL`.
- **Code shown with a question lives in `exercise.code` / `exercise.language`, never in `description`.** `description` is rendered as a plain text node on every frontend screen, so newlines and indentation collapse. `code` is `text` (unlimited); `language` is one of `CodeLanguage` (`src/enums/code-language.enum.ts`), defaulting to `python`. `NextQuestionDto` (`dto/exerciseAndSession/session.dto.ts`) is a separate projection from the `Exercise` entity — a new question field must be added to **both** it and `buildQuestionDto`, or students never receive it.
- **A drafted exercise that talks about code must carry code.** `draft.validator.ts` rejects a draft whose `description` matches `mentionsCode()` (Thai and English cues) while `code` is empty — such a question cannot be answered at all. It first tries to rescue the draft by lifting a fenced code block out of `description` into `code` rather than discarding it.

## Security & data protection (PDPA)

`userprofile` (`server/app/src/entity/userprofile.entity.ts`) holds personal and behavioral data in scope for Thailand's PDPA (Personal Data Protection Act) — `fullName`, `birthDate`, `genderId`, campus/faculty/major, plus derived learning-behavior data (`behaviorScore`, `strengthWeaknessMatrix`, and `conceptMapState` on each of the user's `branch` rows). Treat all of it as PII, not just the obvious name/DOB fields:
...
- `docs/` currently has `to-do.md` (a dated code-review punch list for the Goal admin panel), `home_page_real_data_implementation_plan.md`, `glossary.md` (domain terms: P(L), Progress, not started, …) and `adr/` (decision records, starting with `0001-students-see-progress-not-pl.md`).
- This file is kept in sync with [`GEMINI.md`](GEMINI.md) in this same directory. Update both together.
