...
- **Prerequisite unlock rule**: a skill unlocks only when *all* its parent prerequisites (from `skillPrerequisite`) have reached **100% Progress** in that branch's `conceptMapState` — the frontend's `computeUnlockedSkills`/`canUnlockSkill` check `=== 100`, and `recommendNextSkill`/`getBranchStats` check `pL >= 0.95`. (Earlier versions of this file said ≥60%; no code implements that.)
...
- **Students see Progress, never raw P(L)** ([ADR 0001](docs/adr/0001-students-see-progress-not-pl.md); terms in [`docs/glossary.md`](docs/glossary.md)). Progress = `min(100, round(pL / 0.95 × 100))`, written only by `MasteryState.buildEntry`; a skill with `attemptCount = 0` is shown as "not started". The skill tree reads it from `getBranchSkills`, the Exercise screen from `progress` on `/session/start` and `/session/:id/answer`. Any new screen that shows skill progress must return `SkillProgress` (`MasteryState.toProgress`) rather than let the frontend recompute from `pL`.
