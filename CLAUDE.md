...
- **Goal node** ([ADR 0005](docs/adr/0005-goal-node-derived-on-read.md)): every branch's skill tree ends in a goal node, derived on every read of `GET /branch/:branchId/skills` by `buildGoalNode` (`libs/bkt/goalNode.ts`) and **never stored** — no row, migration or branch-creation hook, so every branch, new or old, has one. The endpoint returns `{ skills, goal }`; keep the goal node out of the skill array, or unlock/picker/exercise/recommendation code will treat it as a practisable skill. It is complete when every `goalSkillRequire` skill has P(L) ≥ 0.95 in that branch (the unlock test) — this repo counts (`masteredCount`/`requiredCount`/`isComplete`); the frontend in `../G06_adaptive_learning` only draws it.
