...
>
> **Follow-up added after execution (Task 5b).** A post-migration audit found the history-driven backfill could not restore one category of data: a pretest seeds an entry for *every* goal skill, including skills the student never answered a question on, and those carry no `history` row — 53 entries across 31 branches. The values are deterministic (`computePL0` with empty stats), so `src/libs/bkt/pretestSeed.ts` (`PretestSeeder.missingEntries`, 5 unit tests) recomputes them and the backfill script gained a second pass that applies them. Verified: 0 missing entries remain, and the branch states now hold 123 skill entries (70 from history + 53 seeded). Prod is untouched by design and will fail until its own migration is run — see Global Constraints.
