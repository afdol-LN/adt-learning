...
4. **Layout and interaction are frontend-only.** The skill tree above is drawn exactly as before. The goal node hangs one row below the deepest skill and is joined only to the **ends of the tree**, meaning the skills that no other skill in the tree builds on. It is centred under them (`layoutGoalNode`, `treeEndSkillIds`). Edges carry no meaning for completion: a required skill in the middle of the tree still counts, even though no edge runs from it to the goal node. Clicking it opens `GoalSidePanel`, which shows goal progress, the "x/y" count, and the required skills. It has no exercise button.
...
- An end of the tree that sits on an upper row, with nothing built on it, gets a long edge down to the goal row.
