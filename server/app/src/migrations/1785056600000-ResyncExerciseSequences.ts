...
export class ResyncExerciseSequences1785056600000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`SELECT setval('exercise_id_seq', (SELECT COALESCE(MAX(id), 1) FROM "exercise"))`);
        await queryRunner.query(`SELECT setval(pg_get_serial_sequence('"exerciseChoice"', 'id'), (SELECT COALESCE(MAX(id), 1) FROM "exerciseChoice"))`);
    }
