...
export class AddExerciseSkillLevelAndType1784470000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `UPDATE "exercise" SET "skill_level" = "level" WHERE "skill_level" IS NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "exercise" ALTER COLUMN "skill_level" SET NOT NULL`,
    );

    await queryRunner.query(
      `CREATE TYPE "public"."exercise_type_enum" AS ENUM('CHOICE', 'FILL_IN_BLANK')`,
    );
    await queryRunner.query(
      `ALTER TABLE "exercise" ADD "type" "public"."exercise_type_enum" NOT NULL DEFAULT 'CHOICE'`,
    );
    await queryRunner.query(
      `UPDATE "exercise" SET "type" = 'FILL_IN_BLANK' WHERE "fillInBlank" IS NOT NULL AND trim("fillInBlank") <> ''`,
    );
  }
